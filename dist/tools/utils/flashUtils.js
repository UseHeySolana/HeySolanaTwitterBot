...
const fetchOraclePrice = (symbol) => __awaiter(void 0, void 0, void 0, function* () {
    const priceFeedId = PRICE_FEED_IDS[symbol];
    if (!priceFeedId) {
        throw new Error(`Price feed ID not found for symbol: ${symbol}`);
    }
    try {
        const hermesPriceFeed = yield hermesClient.getPriceFeeds({
            query: symbol,
            filter: "crypto",
        });
        ...
        const hemrmesPriceUdpate = yield hermesClient.getLatestPriceUpdates([priceFeedId], {
            encoding: "hex",
            parsed: true,
        });
...
exports.POOL_CONFIGS.forEach((poolConfig) => {
    poolConfig.markets.forEach((market) => {
        ...
        if ((targetToken === null || targetToken === void 0 ? void 0 : targetToken.symbol) && (collateralToken === null || collateralToken === void 0 ? void 0 : collateralToken.symbol)) {
...
function getNftTradingAccountInfo(userPublicKey, perpClient, poolConfig, collateralCustodySymbol) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const nftReferralAccountInfo = yield perpClient.provider.connection.getAccountInfo(nftReferralAccountPK);
        ...
        if (nftReferralAccountInfo) {
            ...
            if (nftTradingAccountPk) {
                const nftTradingAccountInfo = yield perpClient.provider.connection.getAccountInfo(nftTradingAccountPk);
                if (nftTradingAccountInfo) {
                    ...
                    const accountExists = yield perpClient.provider.connection.getAccountInfo(nftOwnerRebateTokenAccountPk);
                    if (!accountExists) {
